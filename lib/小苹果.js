var rule = {
    title: '小苹果',
    host: 'https://v.qq.com',
    homeUrl: '/x/bu/pagesheet/list?_all=1&append=1&channel=cartoon&listpage=1&offset=0&pagesize=15&iarea=-1&sort=75',
    ...
    timeout: 5000,
    cate_exclude: '短剧|游戏|全部',
    class_name: '4K电影&4K电视剧&4K动漫',
    class_url: 'movie&tv&cartoon',
    limit: 20,
    play_parse: true,
    lazy: $js.toString(() => {
        try {
            ...
            if (bata.url.includes("http")) {
                input = {header: {'User-Agent': ""},parse: 0,url: bata.url,jx: 0,danmaku: 'http://127.0.0.1:9978/proxy?do=danmu&site=js&url=' + input.split("?")[0]};
            } else {
                input = {header: {'User-Agent': ""},parse: 0,url: input.split("?")[0],jx: 1,danmaku: 'http://127.0.0.1:9978/proxy?do=danmu&site=js&url=' + input.split("?")[0]};
            }
        } catch {
            input = {header: {'User-Agent': ""},parse: 0,url: input.split("?")[0],jx: 1,danmaku: 'http://127.0.0.1:9978/proxy?do=danmu&site=js&url=' + input.split("?")[0]};
        }
    ...
    二级: $js.toString(() => {
        ...
        try {
            ...
            let remarks = json.rec || "";
            let episodeMatch = remarks.match(/(\d+)集/);
            if (episodeMatch) {
                remarks = `更新至${episodeMatch[1]}集`;
            } else if (remarks.includes("全")) {
                remarks = "全集";
            }
        ...
        if (/get_playsource/.test(input)) {
            ...
            indexList.forEach(function(it) {
                ...
                vdata.forEach(function(item) {
                    d.push({title: item.title,pic_url: item.pic,desc: item.episode_number + "\t\t\t播放量：" + item.thirdLine,url: item.playUrl})
                });
            ...
            if (video_lists.length === 1) {
                ...
                video_list.forEach(function(it, idex) {
                    ...
                    QZOutputJson.results.forEach(function(it1) {
                        ...
                        d.push({title: it1.title,pic_url: it1.pic160x90.replace("/160", ""),desc: it1.video_checkup_time,url: url,type: it1.category_map && it1.category_map.length > 1 ? it1.category_map[1] : ""})
                    })
                })
            }
        }
        ...
        let zp = d.filter(function(it) {
            let title = it.title || "";
            let type = it.type || "";
            const excludeKeywords = ["预告", "花絮", "片花", "特辑", "幕后", "采访", "制作", "MV", "主题曲", "剪辑", "合集", "短剧", "精彩", "集锦","盘点", "回顾", "解说", "评测", "反应", "reaction"];
            return !excludeKeywords.some(keyword => title.includes(keyword) || type.includes(keyword));
        });
...
搜索: $js.toString(() => {
    ...
    function vodSearch(keyword, page = 0) {
        return request('https://pbaccess.video.qq.com/trpc.videosearch.mobile_search.MultiTerminalSearch/MbSearch?vplatform=2', {
            body: JSON.stringify({version: "25042201",clientType: 1,filterValue: "",uuid: "B1E50847-D25F-4C4B-BBA0-36F0093487F6",retry: 0,query: keyword,pagenum: page,isPrefetch: true,pagesize: 30,queryFrom: 0,searchDatakey: "",transInfo: "",isneedQc: true,preQid: "",adClientInfo: "",extraInfo: {isNewMarkLabel: "1",multi_terminal_pc: "1",themeType: "1",sugRelatedIds: "{}",appVersion: ""}}),
            headers: {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.139 Safari/537.36','Content-Type': 'application/json','Origin': 'https://v.qq.com','Referer': 'https://v.qq.com/'},
            method: 'POST'
        });
    }
    ...
    function isMainContentOrTheater(title, keyword) {
        if (!title) return false;
        let cleanTitle = title.replace(/|<\/em>/g, '').toLowerCase();
        let cleanKeyword = keyword.toLowerCase();
        if (!cleanTitle.includes(cleanKeyword)) return false;
        const excludeKeywords = ['合集', '预告', '花絮', '片花', '特辑', '幕后', '剪辑', '精彩', '集锦', '盘点', '动漫', '动画','短剧', '采访', '制作', 'mv', '主题曲', '配音', '解说', '评测', '反应', 'reaction', '混剪'];
        if (excludeKeywords.some(excludeWord => cleanTitle.includes(excludeWord))) return false;
        const allowedSuffixes = ['剧场版', '剧场', '电影', '大电影','第一季', '第二季', '第三季', '第四季', '第五季','第一部', '第二部', '第三部', '第四部', '第五部'];
        let baseTitle = cleanTitle;
        let hasAllowedSuffix = false;
        for (let suffix of allowedSuffixes) {
            if (cleanTitle.includes(suffix)) {
                baseTitle = cleanTitle.replace(suffix, '').trim();
                hasAllowedSuffix = true;
                break;
            }
        }
        if (hasAllowedSuffix) return baseTitle.includes(cleanKeyword);
        return cleanTitle === cleanKeyword || cleanTitle === cleanKeyword + ' 电视剧' || cleanTitle === cleanKeyword + ' 电影';
    }
    ...
    try {
        ...
        function processItemList(itemList) {
            if (!itemList) return;
            itemList.forEach(it => {
                if (it.doc && it.doc.id && it.videoInfo && isQQPlatform(it.videoInfo.playSites) && isMainContentOrTheater(it.videoInfo.title, keyword)) {
                    const itemId = it.doc.id;
                    if (!seenIds.has(itemId)) {
                        seenIds.add(itemId);
                        let title = it.videoInfo.title.replace(/|<\/em>/g, '');
                        d.push({title: title,img: it.videoInfo.imgUrl || "",url: itemId,desc: ""});
                    }
                }
            });
        }
        ...
        if (json.data && json.data.areaBoxList) json.data.areaBoxList.forEach(box => processItemList(box.itemList));

    } catch (e) {log("搜索出错: " + e.message);}
    setResult(d);
})
};